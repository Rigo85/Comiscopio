...
std::vector<CanonicalArchiveEntry> sortAndRenameEntries(
    ...
    std::vector<CanonicalArchiveEntry> sortedEntries;
    sortedEntries.reserve(input.size());

    for (std::size_t i = 0; i < input.size(); i++) {
        auto& entry = input[i];
        if (entry.rawPath.empty()) {
            sortedEntries.push_back({entry.archivePath, entry.sortKey, ""});
            continue;
        }
        ...
        char finalName[32];
        std::snprintf(finalName, sizeof(finalName), "%06d%s", static_cast<int>(i), ext.c_str());
        ...
        sortedEntries.push_back({entry.archivePath, entry.sortKey, finalPath});
    }
